...
class SettingsManager:
    ...
    def load(self)->ApplicationSettings:
        ...
        try:
            ...
            if version not in {1,SETTINGS_SCHEMA_VERSION}:raise ValueError("unsupported settings version")
            audio=raw.get("audio",{});visual=raw.get("visual",{});display=raw.get("display",{});input_data=raw.get("input",{}) if version>=2 else {}
            if not all(isinstance(section,dict) for section in (audio,visual,display,input_data)):raise TypeError("settings sections must be objects")
            ...
            muted=audio.get("muted",False);fullscreen=display.get("fullscreen",False);vibration=input_data.get("vibration_enabled",True)
            if not isinstance(muted,bool) or not isinstance(fullscreen,bool) or not isinstance(vibration,bool):raise TypeError("toggle settings must be booleans")
            quality=visual.get("effects_quality","full")
            if quality not in {"full","reduced","off"}:raise ValueError("invalid effects quality")
            return ApplicationSettings(AudioSettings(master_volume=volume("master_volume",1),music_volume=volume("music_volume",.75),sfx_volume=volume("sfx_volume",.85),ambience_volume=volume("ambience_volume",.60),ui_volume=volume("ui_volume",.80),muted=muted),quality,fullscreen,vibration)
