...
class LevelDocument:
 def __init__(self,data:dict,source_path:Path|None=None,npcs:list|None=None):self.data=deepcopy(data);self.source_path=source_path;self.npcs=deepcopy(npcs or []);self.dirty=False;self.selection=None;self.validation_errors=[]
 @classmethod
 ...
 def restore(self,s):self.data=deepcopy(s.data);self.npcs=deepcopy(s.npcs);self.dirty=True
 @property
 ...
 def set_tiles(self,cells:dict[tuple[int,int],int]):
  ...
  self.data["tiles"]=[{"id":value,"position":[x,y],"size":[1,1]} for y,row in enumerate(grid) for x,value in enumerate(row) if value];self.dirty=True
