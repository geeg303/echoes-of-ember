...
class SettingsController:
    ...
    def handle(self,action:MenuAction)->None:
        ...
        if action is MenuAction.CONFIRM:
            ...
            elif item.item_id=="vibration":self.settings.vibration_enabled=not self.settings.vibration_enabled
    ...
    def _reset(self)->None:
        defaults=self.manager.reset();self.settings.audio=defaults.audio;self.settings.effects_quality=defaults.effects_quality;self.settings.fullscreen=defaults.fullscreen;self.settings.vibration_enabled=defaults.vibration_enabled;self.audio.settings=defaults.audio;self.audio._apply_volumes();self.effects.set_quality(EffectQuality.FULL);self.host.set_fullscreen(False);self._rebuild("reset")
    def close(self)->None:self.manager.save(self.settings);self.audio.play_sound("ui_cancel");self.host.close_settings()
    def draw(self,surface)->None:
        surface.fill((12,18,34));draw_menu(surface,self.title,self.font,self.small,"SETTINGS",self.menu,"Immediate application · Saved on exit",f"[{self.host.input.get_prompt(Action.CONFIRM)}] SELECT   [{self.host.input.get_prompt(Action.BACK)}] BACK")
